...
LEFT OFF HERE
Could easily look at 2019-2022 together for number of germs in Y1 or Y2. If Obs Year == Add_Year, then Germ_Year = 1
...
# you actually want to use previously reduced data
blue_19_23 <- read_csv("germ_19_23.csv")

# Isolate by year
blue_19 <- blue_19_23 %>% 
  filter(Add_Year == 2019 & Germ.binom == 1) %>% 
  mutate(Germ_Year = year(Obs_Date))
...
There's clear evidence for limiting to first two years. Let's make it official. Go back to top of this code chunk and code for Year 1 Year 2 and then graph and get percentages.
