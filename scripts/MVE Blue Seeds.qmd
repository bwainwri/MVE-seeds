...
write_csv(blue_19_23, "germ_19_23.csv")
...
clim_sum <- met_00_23_50 %>% 
  ...
  summarise(sum_pp = sum(Precipitation),
            daily_min = min(Min_Temp_C),
            daily_max = max(Max_Temp_C))

clim_sum_pp <- clim_sum %>% 
  group_by(Month) %>% 
  summarise(mean_pp = mean(sum_pp, na.rm = TRUE),
            sd_pp = sd(sum_pp, na.rm = TRUE),
            n_pp = n()) %>%
  mutate(se_pp = sd_pp / sqrt(n_pp),
         lower_ci_pp = mean_pp - qt(1 - (0.05 / 2), n_pp - 1) * se_pp,
         upper_ci_pp = mean_pp + qt(1 - (0.05 / 2), n_pp- 1) * se_pp)

ggplot(clim_sum_pp, aes(x = Month, y = mean_pp)) + 
  geom_point() + 
  geom_smooth() +
  labs(title = "Average Daily Precip (mm)")
  

clim_sum_max_t <- clim_sum %>% 
  group_by(Month) %>% 
  summarise(mean_max_t = mean(daily_max, na.rm = TRUE),
            sd_max_t = sd(daily_max, na.rm = TRUE),
            n_max_t = n()) %>%
  mutate(se_max_t = sd_max_t / sqrt(n_max_t),
         lower_ci_max_t = mean_max_t - qt(1 - (0.05 / 2), n_max_t - 1) * se_max_t,
         upper_ci_max_t = mean_max_t + qt(1 - (0.05 / 2), n_max_t- 1) * se_max_t)

clim_sum_min_t <- clim_sum %>% 
  group_by(Month) %>% 
  summarise(mean_min_t = mean(daily_min, na.rm = TRUE),
            sd_min_t = sd(daily_min, na.rm = TRUE),
            n_min_t = n()) %>%
  mutate(se_min_t = sd_min_t / sqrt(n_min_t),
         lower_ci_min_t = mean_min_t - qt(1 - (0.05 / 2), n_min_t - 1) * se_min_t,
         upper_ci_min_t = mean_min_t + qt(1 - (0.05 / 2), n_min_t- 1) * se_min_t)
...
Ok time to graph. Graph each one individually first.

```{r}
ggplot(clim_sum_3, aes(x = Month, y = mean_pp)) + 
  geom_point() + 
  geom_smooth() +
  labs(title = "Average Monthly Precip (mm)")
  
ggplot(clim_sum_3, aes(x = Month, y = mean_max_t)) + 
  geom_point() + 
  geom_smooth() +
  labs(title = "Average Daily Temp Max (C)")
  
ggplot(clim_sum_3, aes(x = Month, y = mean_min_t)) + 
  geom_point() + 
  geom_smooth() +
