...
## Notes from 10/28 with Jenn
- Get recent soil moisture from EDI
- Get the rest of the germ data for 2023 into 2024, ignore 2024 cohort
- Add confidence bands around climate graph -- jenn to send standard
- Use MVE color scheme and same treatment names as recent paper
- Why did blue do better in 2022
- For germination bar graphs use ratio of blue to black germination then create line graphs over time where each line is a different treatment and each point is the ratio value for each year
- In her experience year rarely explains anything but things related to climate do
- Look at the germination ratio related to climate
- For the germination and soil moisture model, are the slopes of blue and black different in the interaction model? You can use emtrends for slopes.
- Intercepts are overall who is doing better, slope is how sensitive
- right now species estimate (in non interactive model) says the overall means across soil moisture
Survival: cut off strictly at 2 years
- Doing this may improve assuption violations for Add Year
- Need better output for survival model. Look at contrasts -- test for essect of mean treatment within variance treatments then flip the order. Get something that looks like anova table for survival
- Get an emmeans or contrast statement. Find a way to get a pairwise contrast statement to figure out which differ
- For survival and soil moisture -- use mortality events like you did for germination events, parallel to germination
- Whiplash graph would suggest that blue grama germination is less variable overtime. blue grama should have more stable germination over time, black should have higher cv
- Survival + legacy: run the survival curve for each cohort in each plot and take ratio at 250 days. Use that proportion and recreate legacy graphs
- Don't look at biomass, then stear clear of competition
- In recent MVE paper on cover, black grama is not responding to treatments, and increasing overall slightly. blue grama is declining in drier more variable plots and significantly declining overall over time
- Use this in discussion to compare and contrast the effects of treatments at different life history stages.
...
met_event_sum_6 <- met_event_6 %>% 
  ...
  summarise(sum_pp = sum(Precipitation, na.rm = TRUE))
```
Graphs below show the 6 germination events (no soil moisture data for 2023) from 2019-2022. Vertical black line is germination event (Observation Date). Shows precipitation (yellow), average daily soil moisture (purple), and maximum daily soil moisture (turquoise) for the 28 days leading up to the Observation date.
...
```{r}
summary(blue_19_23) # whether a toothpick germinated or not
# head(blue_19_23)
# str(blue_19_23)
...
```{r}
summary(bluebyyear) # whether a toothpick germinated in it's first or second year
...
I want a graph where the x-axis is days of the year and there are three lines, each with confidence bands: average daily precip, daily high temperature, daily low temperature. Adding confidence bands really distorts the graph because precipitation has such high variability and temperature doesn't. 
...
emtrends(model, var="soilmoist", ~species) -- use this for looking at slopes of interactive model of soil moisture and species
...
All of these graphs makes sense except for the September prior graph. I would expect a drier prior year would have lower cover and lead to high germination. But maybe this is explained by certain species doing well in dry conditions? 
