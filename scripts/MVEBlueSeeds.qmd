...
mall5 <- glmer(Germ.binom ~ mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block), family = binomial, data = blue_yr_trt)

m_pp <- glmer(Germ.binom ~ sum_pp_sc + (1|Block), family = binomial, data = blue_yr_trt)

mall6 <- glmer(Germ.binom ~ mean_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block), family = binomial, data = blue_yr_trt)

mall7 <- glmer(Germ.binom ~ var_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block), family = binomial, data = blue_yr_trt)

ICtab(m0,m1,m2,m3,mall,mall2,mall3,mall4, mall5, m_pp, mall6, mall7)
Anova(mall)
summary(mall)
mall
...
Model with a four way interaction between species, mean_treatment, var_treatment, and total annual precipitation (scaled). This model fails to converge but not by much. Model also not overdispersed. Proceed with interpretations! Model with three way interaction between mean variance and sum_pp is next best. 
...
#lrtest(m_mons_max, m3)
Anova(m_mons_max_x_sp)
summary(m_mons_max_x_sp)
#mall
...
summary(survfit(Surv(Surv_days, Status) ~ Species, data = surv), times = 365.25)
