...
Create a dataset limited to the major soil moisture germination events.
Isolate each Observation Year to the two (or three) events with the most germination. Then pair with soil moisture data from plots.
...
mall <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block), family = binomial, data = blue_yr_trt)
...
mall4 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Block), family = binomial, data = blue_yr_trt)

ICtab(m0,m1,m2,m3,mall,mall2,mall3,mall4)
Anova(mall)
summary(mall)
mall
...
Model with a four way interaction between species, mean_treatment, var_treatment, and total annual precipitation (scaled). This model fails to converge but not by much. Model also not overdispersed. Proceed with interpretations!

Model shows that effect of climate treatments varies by species and is dependent on annual precipitation.
...
-   Figure out how to graph in a way that reflects 4 way interaction
-   first at least add total annual precipitation to each year in figure

## Model Soil Moisture Germination Events

LEFT OFF HERE Model & Graph Soil Moisture Germination Events
...
# Filter sm_t by each date and 14 days before and after. 
# (perfect opp for a loop)

sm_t_event_1 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[1]-15) & Date < (events[1]+15))) %>%
...
ggplot(sm_event_sum_1, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_1, aes(x = Date, y = max_sm)) +
  geom_line()

sm_t_event_2 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[2]-14) & Date < (events[2]+14))) %>%
  filter(sensor == "VWC" & depth == 12) %>% 
...
ggplot(sm_event_sum_2, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_2, aes(x = Date, y = max_sm)) +
  geom_line()

sm_t_event_3 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[3]-14) & Date < (events[3]+14))) %>%
  filter(sensor == "VWC" & depth == 12) %>% 
...
ggplot(sm_event_sum_3, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_3, aes(x = Date, y = max_sm)) +
  geom_line()

sm_t_event_4 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[4]-14) & Date < (events[4]+14))) %>%
  filter(sensor == "VWC" & depth == 12) %>% 
...
ggplot(sm_event_sum_4, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_4, aes(x = Date, y = max_sm)) +
  geom_line()

sm_t_event_5 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[5]-14) & Date < (events[5]+14))) %>%
  filter(sensor == "VWC" & depth == 12) %>% 
...
ggplot(sm_event_sum_5, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_5, aes(x = Date, y = max_sm)) +
  geom_line()

sm_t_event_6 <- sm_t %>%
  group_by(Date) %>%
  filter(any(Date > (events[6]-14) & Date < (events[6]+14))) %>%
  filter(sensor == "VWC" & depth == 12) %>% 
...
ggplot(sm_event_sum_6, aes(x = Date, y = avg_sm)) +
  geom_line()
ggplot(sm_event_sum_6, aes(x = Date, y = max_sm)) +
  geom_line()

# Might need to go further back than 14 days
...
ggplot(met_event_sum_1, aes(x = Date, y = sum_pp)) +
  geom_line()

# I want 6 graphs, with two axes, soil moisture one and precip on the other. Two lines are max and avg sm and one is total daily precip. And a vertical line at the Obs_Date and maximum either soil moisture or precip. Go back and go 20 days back and 4 days forward
