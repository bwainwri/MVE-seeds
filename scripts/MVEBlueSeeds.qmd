...
Try simplifying random effect structure (limit to 1|Quad_ID and/or 1|Obs_Year)
Try combining mean and variance treatment into one.
...
Try a fixed effects model only
...
m1 <- glmer(Germ.binom ~ 1 + (1|Block/plot/Corner) + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

m2 <- glmer(Germ.binom ~ 1 + (1|Quad_ID) + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

m3 <- glmer(Germ.binom ~ 1 + (1|Quad_ID_ne_yr) + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

m4 <- glmer(Germ.binom ~ 1 + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

m5 <- glmer(Germ.binom ~ 1 + (1|Quad_ID), family = binomial, data = blue_yr_trt)

mall <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block/plot/Corner), family = binomial, data = blue_yr_trt, control = glmerControl(optimizer = "bobyqa"))

mall2 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Block/plot/Corner), family = binomial, data = blue_yr_trt)

mall3 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc_2 + (1|Block/plot/Corner) + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

mall4 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Block/plot/Corner) + (1|Obs_Year_sc_2), family = binomial, data = blue_yr_trt)

mall5 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Quad_ID_ne_yr) + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

mall6 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Quad_ID), family = binomial, data = blue_yr_trt, control = glmerControl(optimizer = "bobyqa"))
summary(mall6)

mall7 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

mall8 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc) + (1|Block), family = binomial, data = blue_yr_trt)

mall9 <- glmer(Germ.binom ~ Species*mean_treatment*var_treatment*sum_pp_sc + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)

mall10 <- glmer(Germ.binom ~ Species*meanvar*sum_pp_sc + (1|Obs_Year_sc), family = binomial, data = blue_yr_trt)


lrtest(m4,mall7)
lrtest(m0,mall3)

ICtab(m0,m1,m2,m3,m4,m5,mall,mall2, mall3, mall4,mall5,mall6,mall7)
Anova(mall)
summary(mall)
vif(mall)
...
Model with a four way interaction between species, mean_treatment, var_treatment, and total annual precipitation. But this model fails to converge even after rescaling variable and many variable have variance inflation factors > 10. 

Try doing a model for each species. LEFT OFF HERE OVERWHELMED.
...
Isolate each Observation Year to the two (or three) events with the most germination. The pair with soil moisture data from plots.

```{r}
str(blue_yr_trt)

germ_sum <- blue_yr_trt %>% 
