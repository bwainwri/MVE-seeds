...
### Create soil moisture variables and pair with germ data
...
sm_pp_events <- sm_pp_events %>% 
  select(Date,
         Obs_Date,
         sum_day_pp) %>% 
  distinct()

sm_event_sum <- sm_events %>% 
  group_by(Date, Plot) %>% 
  summarise(avg_day_sm = mean(value, na.rm = TRUE),
            max_day_sm = max(value, na.rm = TRUE),
            Obs_Date = mean(Obs_Date),
            sum_day_pp = mean(sum_day_pp))
...
sm_event_vars <- left_join(sm_event_avg_7_max, sm_event_avg_7, by = join_by(Obs_Date, Plot))

# average of maximums for Obs_Date-Maximum Date
pp_events_sum <- sm_pp_events %>% 
  group_by(Obs_Date) %>% 
...
sm_event_vars_2 <- left_join(sm_event_vars, sm_event_avg_max_mons, by = join_by(Obs_Date, Plot))
...
## LEFT OFF HERE
## Clean this up and retrace our steps because 7-05 date has really low soil moisture
